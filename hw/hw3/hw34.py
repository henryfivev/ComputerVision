...
# 创建数据加载器
labeled_loader = DataLoader(labeled_dataset, batch_size=batch_size, shuffle=True, num_workers=1, pin_memory=True)
unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=batch_size, shuffle=True, num_workers=1, pin_memory=True)
...
def train(epoch):
    ...
    for batch_idx, ((inputs_x, targets_x), (inputs_u, _)) in enumerate(zip(labeled_loader, unlabeled_loader)):
        ...
        targets_x = torch.zeros(64, 10).scatter_(1, targets_x.view(-1, 1), 1).to(device)  # 将 targets_x 移动到设备上

        optimizer.zero_grad()

        logits_x = model(inputs_x)
        loss = mixmatch_loss(inputs_x, targets_x, inputs_u, targets_x, logits_x)
